...
public enum Constants: Sendable {
    // SSH2VPN fork: spoof a Termux admin's OpenSSH 9.6 banner. TSPU reads
    // this line in plaintext before any encryption exists.
    static let version = "SSH-2.0-OpenSSH_9.6"
    ...
    public static let bundledTransportProtectionSchemes: [(NIOSSHTransportProtection & _NIOSSHSendableMetatype).Type] =
        [
            // SSH2VPN fork: aes128-gcm first, matching OpenSSH 9.6's
            // relative cipher order (chacha20, ctr modes, aes128-gcm,
            // aes256-gcm — the fork only implements the gcm pair).
