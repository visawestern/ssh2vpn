...
extension SSHKeyExchangeStateMachine {
    ...
    // SSH2VPN fork: order mirrors OpenSSH 9.6's RELATIVE preference among
    // the primitives implemented here (curve25519, ecdh-nistp256/384/521).
    // PQ-hybrid and DH-group KEX names are NOT advertised: offering an
    // algorithm we cannot complete would fail negotiation outright.
    ...
    /// SSH2VPN fork: ecdsa-nistp256 before nistp384, matching OpenSSH 9.6's
    /// relative order among plain (non-cert) host key algorithms.
