...
extension NIOSSHHandler {
    ...
    private func createPendingChannelsIfPossible() {
        guard self.pendingChannelInitializations.count > 0 else {
            // No work to do
            return
        }

        guard let multiplexer = self.multiplexer, !self.stateMachine.disconnected else {
            // No parent to create channels on (removed or disconnected):
        ...
        // SSH2VPN fork: create only when strictly active (past authentication
        // AND past any rekey), NOT merely hasActivated. The state machine
        // throws protocolViolation for channel messages sent mid-rekey, and
        // the child-channel open path swallows that error (nil write
        // promise), so an open attempted during rekey wedged forever: no
        // open-confirmation ever arrives, yet the promise never fails.
        // Queued opens now wait for the return to .active; the
        // channelReadComplete that processes the peer's NEWKEYS flushes them
        // in the same turn, so no extra inbound traffic is needed. (Global
        // requests already gated on isActive — this aligns channel opens
        // with them.)
        guard self.stateMachine.isActive else {
            // Authenticating or mid-rekey: leave queued.
            return
        }
...
extension NIOSSHHandler {
    ...
    /// Sends an arbitrary SSH global request by name (`SSH_MSG_GLOBAL_REQUEST`).
    ///
    /// SSH2VPN fork: `SSHMessage` itself stays internal (making it public
    /// would leak ~30 message types), so this is the public primitive. It is
    /// the protocol-level keepalive: `sendGlobalRequest(name:
    /// "keepalive@openssh.com", payload: empty, wantReply: true)` emits
    /// exactly what `ssh -o ServerAliveInterval` sends, and stock sshd
    /// answers `SSH_MSG_REQUEST_SUCCESS` — no channel open/close dance, no
    /// `MaxSessions` slot burned per ping.
    ///
    /// This function is **not** thread-safe: it may only be called from on the channel.
    ///
    /// - parameters:
    ///     - name: The request name, e.g. `keepalive@openssh.com`.
    ///     - payload: Request-specific payload (empty for keepalive).
    ///     - wantReply: Whether the server must reply (true for keepalive).
    ///     - promise: Fulfilled with the reply payload on
    ///         `SSH_MSG_REQUEST_SUCCESS`, failed on `SSH_MSG_REQUEST_FAILURE`
    ///         or send error. Nil = fire-and-forget (ordering against other
    ///         global requests is still preserved by the pending queue).
    public func sendGlobalRequest(
        ...
        self.sendGlobalRequestMessage(
            .init(wantReply: wantReply, type: .unknown(name, payload)),
            promise: promise
        )
    }
...
extension NIOSSHHandler {
    ...
    /// Initiates a key re-exchange (`SSH_MSG_KEXINIT` mid-connection, RFC 4253 §9)
    /// on an established connection.
    ///
    /// SSH2VPN fork: OpenSSH rekeys by default after 4GB or 1h (`RekeyLimit`);
    /// a connection that never rekeys is a long-lived-session fingerprint, so
    /// the pool calls this on the same schedule. Safe to call while channels
    /// are open — the state machine queues channel traffic during the exchange.
    ///
    /// This function is **not** thread-safe: it may only be called from on the channel.
    /// Check `isReadyForRekey` first: calling this while still authenticating
    /// (or mid-rekey) traps — the underlying state machine has no error path.
    ///
    /// - throws: If the connection is not in a state that can begin rekeying
    ///     (not yet authenticated, already rekeying, or closed).
    public func rekey() throws {
        try self._rekey()
    }

    /// SSH2VPN fork: true only while fully authenticated and NOT already
    /// rekeying/disconnecting — i.e. `rekey()` is safe to call right now.
    /// The check and the `rekey()` call must happen in the same event-loop
    /// turn (the loop is serial, so nothing can interleave between them).
    ///
    /// This property is **not** thread-safe: it may only be read from on the channel.
    public var isReadyForRekey: Bool {
        self.stateMachine.isActive
    }
}
