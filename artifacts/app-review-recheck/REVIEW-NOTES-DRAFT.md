...
Host: [SSH_HOST] / Port: 22 / Username: [SSH_USER] / Password: [SSH_PASS]
Auth type: Password (leave the private-key field empty).
Steps: open the app → Add Server → enter the above → Save → tap the power button → status becomes PROTECTED.

Free tier: every install includes 1 free hour of tunnel time, counted in real
time from the first launch (even with the VPN off). The +3h button plays a
