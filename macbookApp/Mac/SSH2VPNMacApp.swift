...
/// macOS-порт SSH2VPN (Intel x86_64 + ARM): то же приложение, тот же
/// PacketTunnel-расширение, те же технологии, что на iOS.
///
...
struct SSH2VPNMacApp: App {
    ...
    /// MAC: окно фиксированного размера — высота 90% видимой высоты экрана,
    /// ширина пропорциональна iPhone 16 Pro Max (440x956 pt). Ресайз запрещён.
...
final class AppModel: ObservableObject {
    ...
    init() {
        ...
        // MAC: NSApplication вместо UIApplication; idle-lock на macOS нет.
    ...
    /// MAC: на macOS нет idle-timer API (и он не нужен — это не телефон).
    /// Метод оставлен, чтобы общие call sites не расходились с iOS.
    ...
    func connect(manual: Bool = true) {
        ...
        guard quota.allowsConnection(now: Date()) else {
            ConsoleLogStore.shared.log(level: .error, tag: "QUOTA", message: "Connect blocked: free time exhausted — go Unlimited for unlimited time (no rewarded ads on macOS)")
        ...
        // MAC: system extension должен быть активирован (и один раз разрешён
        // в System Settings), иначе startVPNTunnel не с кем разговаривать.
        // Гейт ДО beginConnection и ВНЕ automation/breaker: неодобренная
        // экстеншн — не "упавший туннель" и не должна кормить счётчики.
        Task { @MainActor [weak self] in
            guard let self else { return }
            if let gateError = await MacSystemExtensionGate.shared.ensureActive() {
                self.connection = .failed(gateError)
                return
            }
            do { try self.saveServer(selected) } catch { self.connection = .failed(error.localizedDescription); return }
...
private enum SettingsStore {
    // MAC: app-group suite "group.com.sshtunnel.shared" needs the entitlement
    // and a provisioned profile. If it's not available (CFPrefs warning in
    // console), silently fall back to standard UserDefaults to avoid spam.
    nonisolated(unsafe) private static let defaults: UserDefaults = {
        let suite = UserDefaults(suiteName: "group.com.sshtunnel.shared")
        // Probe write — if suite is broken, use standard instead.
        let testKey = "__suite_probe__"
        if let s = suite, s.string(forKey: testKey) == nil {
            s.set("ok", forKey: testKey)
            if s.string(forKey: testKey) == "ok" {
                s.removeObject(forKey: testKey)
                return s
            }
        }
        return .standard
