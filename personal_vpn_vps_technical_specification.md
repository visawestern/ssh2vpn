...
локальная wall-clock квота бесплатного времени (`QuotaLedger`, Keychain,
enforcement в расширении):

```text
1 час бесплатно — грант при ПЕРВОМ использовании
(первый тап Connect или первая rewarded-реклама, не install)
+3 часа за просмотр rewarded-рекламы (Google AdMob)
(не чаще 1 раза в час, потолок 12 часов суммарно в моменте)
```

Время тикает в реальном времени даже при выключенном VPN.
Разовая покупка Unlimited снимает лимит и убирает рекламу.
Подключение блокируется, а активный туннель отключается при
исчерпании квоты.

> Решение владельца: AdMob остаётся как основа монетизации free-tier
> (покупки — у меньшинства). Compliance-модель: реклама показывается
> только при отключённом VPN, порядок согласий — UMP → ATT →
> MobileAds.start, трекинг задекларирован (NSPrivacyTracking=true).
...
└── Free-time quota (1 ч при первом использовании +3 ч за rewarded-рекламу, cap 12 ч)
...
- free-time quota — ✅ реализовано как 1 ч first-use грант + rewarded-ad (+3 ч, cap 12 ч) вместо backend-trial (см. §2.1).
